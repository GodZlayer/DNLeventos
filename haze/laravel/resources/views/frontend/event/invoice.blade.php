    ...
    <div class="row">
      <div class="col-lg-12">
            ...
            <div class="p-3 border mt-5 mb-2">
              ...
              <p>
                {{ __('Payment Method') . ': ' }} <span
                  class="text-muted">{{ is_null($bookingInfo->paymentMethod) ? '-' : $bookingInfo->paymentMethod }}</span>
              </p>
                          ...
                          @if ($ticketContent && $ticket)
                            <small>
                              ...
                              @if ($ticket->pricing_type == 'variation')
                                @php
                                  $varition_key = App\Models\Event\VariationContent::where([['ticket_id', $ticket->id], ['name', $variation['name']]])->select('key')->first();
                                  $de_lang = App\Models\Language::where('id', $ticket->default_language)->first();
                                @endphp
                                <br> <small><em>{{ $de_lang->code }}: {{ $varition_key->key }}</em></small>
                              @endif
                            </small>
                          @else
                            {{ __('Ticket Not Found') }}
        ...
        <div class="clearfix">
          <div class="float-left px-1" style="width: {{ $w_47 }}">
            <div class="p-3 border mt-3">
              <h6 class="mt-2 mb-3">{{ __('Tickets') }}</h6>
              @foreach ($bookingInfo->tickets as $ticket)
                @php
                  $ticketContent = App\Models\Event\TicketContent::where([['ticket_id', $ticket->ticket_id], ['language_id', $currentLanguageInfo->id]])->first();
                  if (empty($ticketContent)) {
                      $ticketContent = App\Models\Event\TicketContent::where([['ticket_id', $ticket->ticket_id]])->first();
                  }
                  $qrCodePath = public_path('assets/admin/qrcodes/' . $ticket->id . '.svg');
                @endphp

                <div class="d-flex justify-content-between">
                  <span>
                    {{ $ticketContent->title }} - {{ $ticket->id }}
                  </span>
                  <img src="{{ $qrCodePath }}" alt="QR Code" style="width: 60px; height: 60px;">
                </div>
              @endforeach
            </div>
          </div>
        ...
        <div class="clearfix">
          <div class="float-left px-1" style="width: {{ $w_47 }}">
            <div class="p-3 border mt-3 mb-5">
              <h6 class="mt-2 mb-3">{{ __('Terms and Conditions') }}</h6>
              <p>
                {{ __('Please make sure to bring this invoice to the event. The QR code is your ticket and will be scanned at the entrance.') }}
              </p>
              <p>
                {{ __('For any questions, please contact us at') }} <a href="mailto:{{ $websiteInfo->contact_email }}">{{ $websiteInfo->contact_email }}</a>
              </p>
            </div>
          </div>
        </div>
