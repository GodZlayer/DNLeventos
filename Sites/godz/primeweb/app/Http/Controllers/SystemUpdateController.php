...
class SystemUpdateController extends Controller {
    ...
    public function index() {
        $system_version = CachingService::getSystemSettings('system_version');
        $system_version = getSettings('system_version');
    ...
    public function update(Request $request) {
        ...
        try {
            ...
            $app_url = preg_replace('#^https?://#i', '', $app_url);
             $system_version = getSettings('system_version');
            $current_version = $system_version['system_version'];
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL            => 'https://wrteam.in/validator/prime_web_validator?purchase_code=' . $request->purchase_code . '&domain_url=' . $app_url,
